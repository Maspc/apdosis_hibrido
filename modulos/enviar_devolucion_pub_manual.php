<?php
	...
	class enviar_dpm{
		...
		public static function select1($medicamento_id,$cargo) {
			$reg = conexion::sqlGet("select devolucion, cantidad from 
			factura_detalle where medicamento_id= 
			'".$medicamento_id[$k]."' and factura =
			'".$cargo."' ");
			return $reg;
		}
		...
		public static function insert1($fecha_creacion,$stat,$userid,$bodega,$motivo,$caja_id,$cargo) {
			...
			medico, no_cama, estado, fecha_creacion, stat,
			ordenado_por, bodega, motivo, publico, caja_id) 
			select historia, factura, medico, no_cama, 'I', 
			'".$fecha_creacion."', 
			'".$stat."', 
			'".$userid."', 
			'".$bodega."', 
			'".$motivo."', 'S', 
			'".$caja_id."' from factura where factura =
			'".$cargo."'";
			conexion::trQry($sql);
			return 1;
		}
		...
		public static function select4($medicamento_id) {
			$reg = conexion::sqlGet("select costo_unitario from medicamentos where codigo_interno =
			'".$medicamento_id."'");
			return $reg;
		}
		...
		public static function update1($devol,$medicamento_id,$cargo) {
			...
			'".$devol."' where medicamento_id= 
			'".$medicamento_id."' and factura =
			'".$cargo."' ";
			conexion::trQry($sql);
			return 1;
		}
		...
		public static function update3($precio_venta1,$z) {
			$sql ="update devolucion set total =
			'".$precio_venta1."' where devolucion =
			'".$z."'";
			conexion::trQry($sql);
			return 1;
		}
		public static function update4($precio_venta1,$z) {
			...
			a.saldo_actual = a.saldo_actual - 
			'".$precio_venta1."' where c.devolucion = 
			'".$z."' and c.factura = b.factura and a.id_cliente = b.codigo_cliente and b.credito > 0";
			conexion::trQry($sql);
			return 1;
		}
		public static function select6($z) {
			$reg = conexion::sqlGet("SELECT medicamento, forma_farma, dosis, horas, dias, linea, cantidad FROM devolucion_detalle where devolucion =
			'".$z."'");
			return $reg;
		}
		public static function select7($devolucion) {
			$reg = conexion::sqlGet("select factura from devolucion where devolucion = 
			'".$devolucion."'");
			return $reg;
		}
		public static function select8($devolucion) {
			$reg = conexion::sqlGet("SELECT sum(precio_venta) as total from devolucion_detalle where devolucion =
			'".$devolucion."' and no_aceptada != 'S'");
			return $reg;
		}
		public static function select9($factura) {
			$reg = conexion::sqlGet( "SELECT total, fecha_creacion, factura, historia, no_cama from factura where factura =
			'".$factura."'");
			return $reg;
		}
		...
		public static function select11($devolucion) {
			$reg = conexion::sqlGet("select factura_fiscal, equipo_fiscal,
			archivo_fiscal, FA, factura from devolucion where devolucion =
			'".$devolucion."' ");
			return $reg;
		}
		public static function select12() {
			$reg = conexion::sqlGet("select nombre_carpeta, nombre_carpeta2 
			from impresoras_fiscales where tipo_impresion = 'DEP'");
			return $reg;
		}
		...
		public static function select14($id) {
