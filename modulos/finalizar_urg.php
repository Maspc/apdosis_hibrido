<?php
	...
	class finalizar_u{
		...
		public static function select1($factura) {
			$reg = conexion::sqlGet("select estado_factura, procesado_por 
			from factura where factura = '".$factura."'");
			return $reg;
		}
		public static function select2($factura) {
			$reg = conexion::sqlGet("select bodega from factura where
			factura = '".$factura."'");
			return $reg;
		}
		public static function select3($factura) {
			$reg = conexion::sqlGet("SELECT factura_detalle.medicamento,
			factura_detalle.forma_farma, formas_farmaceuticas.descripcion, factura_detalle.dosis, factura_detalle.horas, factura_detalle.dias, factura_detalle.linea, factura_detalle.cargo, factura_detalle.cantidad,
            medicamentos.precio_unitario,
			grupo_de_medicamentos.costo_adicional,
			grupo_de_medicamentos.insumo_prep, 
			...
			factura_detalle.average,
			grupo_de_medicamentos.medicamento_insumo, 
			grupo_de_medicamentos.medicamento_insumo_2,
			grupo_de_medicamentos.no_paga, medicamentos.costo_unitario, 
			factura.stat_inicio FROM factura_detalle, 
			formas_farmaceuticas, medicamentos, registro_detalle,
			grupo_de_medicamentos, impuesto, factura 
            where factura.factura = '".$factura."'
            and factura.factura = factura_detalle.factura
            and factura_detalle.forma_farma = 
			formas_farmaceuticas.codigo_forma
            and factura_detalle.medicamento_id = 
			medicamentos.codigo_interno
            ...
            and medicamentos.grupo_medicamento =
			grupo_de_medicamentos.codigo_grupo
		...
		public static function select4($factura) {
			$reg = conexion::sqlGet("select max(linea) + 1 as 
			linea_siguiente from factura_detalle where factura =
			'".$factura."'");
			return $reg;
		}
		public static function select5($medicamento_insumo) {
			$reg = conexion::sqlGet("select codigo_interno,nombre_generico
			as medicamento from medicamentos where codigo_interno =
			'".$medicamento_insumo."'");
			return $reg;
		}
		public static function insert1($medicamento_ins,$medicamento_id_ins, $linea_siguiente, $costo_adicional,$cantidad_insumo,$costo_adicional_c,$factura, $medicamento_id,$historia,$tratamiento,$cargo,$linea2) {
			...
			medicamento_id,linea, precio_unitario, cantidad, 
			precio_venta, estado_producto,factura,forma_farma, 
			med_insumo, historia, tratamiento, cargo, linea_adic)
            values ('".$medicamento_ins."',
			'".$medicamento_id_ins."', 
			'".$linea_siguiente."', 
			'".$costo_adicional."',
			'".$cantidad_insumo."',
		...
		public static function update2($cantidad,$medicamento_id,$bodega) {
		...
		public static function select6($factura,$medicamento_id) {
			...
			cantidad from lotes_x_factura where factura =
			'".$factura."' and medicamento_id =
			'".$medicamento_id."'");
			return $reg;
		}
		public static function update4($cantidad,$medicamento_id,$lote) {
		...
		public static function update5($medicamento_id,$historia,$tratamiento,$cargo) {
			$sql ="update registro_detalle set estado = 'F', 
			cantidad_de_dosis = 0 where medicamento_id =
		...
		public static function update6($historia,$tratamiento,$cargo) {
		...
		public static function select8($factura) {
			...
			factura_detalle.costo_insumo + factura_detalle.impuesto ) 
			as precio_unitario, medicamento_id, medicamento,historia,
			medicamentos.tipo_de_dosis from factura_detalle,
			medicamentos, grupo_de_medicamentos where factura = 
			'".$factura."' and factura_detalle.medicamento_id = 
			medicamentos.codigo_interno and estado_producto != 'X' 
			and medicamentos.grupo_medicamento = 
			grupo_de_medicamentos.codigo_grupo order by 
			factura_detalle.linea_adic");
			return $reg;
		}
		public static function update8($documento,$factura) {
			$sql ="update factura set FA=
			'".$documento."', estado_envio = 'S', hora_primer_fa = 
			'".date('Y-m-d H:i',time())."' where factura =
			'".$factura."'";
			conexion::trQry($sql);
			return 1;
		}
		public static function insert2($documento_f,$factura) {
			$sql ="insert into logs_errores (codigo_error, id_interno_fac) values ('".$documento_f."', '".$factura."')";
			conexion::trQry($sql);
			return 1;
		}
		
		public static function update9($documento_f,$factura) {
			$sql = "update factura set FA='".$documento_f."', estado_envio = 'S', hora_primer_fa = '".date('Y-m-d H:i',time())."' where factura = '".$factura."'";
			conexion::trQry($sql);
			return 1;
		}
		public static function insert3($documento_f,$factura) {
			$sql ="insert into logs_errores (codigo_error, id_interno_fac) values ('".$documento_f."', '".$factura."')";
			conexion::trQry($sql);
			return 1;
		}
		public static function select9($factura) {
			...
			registro_detalle.cantidad_de_dosis as cant_total, 
			factura_detalle.observacion, medicamentos.tipo_de_dosis, 
			factura_detalle.preparacion, medicamentos.grupo_medicamento,
			impuesto.factor  FROM factura_detalle, formas_farmaceuticas, 
