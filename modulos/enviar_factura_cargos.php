<?php
	...
	class enviar_fc{
		public static function update1($credito,$codigo_cliente) {
			...
			'".$credito."' where id_cliente = '".$codigo_cliente."'";
			conexion::trQry($sql);
			return 1;
		}
		public static function select1($MM_iduser) {
			$reg = conexion::sqlGet("select a.caja_id, b.nombre from cajas_usuario
			a, cajas b where a.usuario =
		...
		public static function insert1($userid, $identificacion, $codigo_cliente, $jubilado, $codigo_aseguradora, $porcentaje_desc, $total, $sub_total, $itbms_total, $descuento_total,$efectivo, $tarjeta_credito, $clave, $credito, $saldo_pendiente, $cheque, $no_cheque, $nombres_banco, $ref_tdb, $ref_tdc, $vuelto, $caja_id, $nombre_cliente) {
            ...
            id_paciente, codigo_cliente, jubilado, codigo_aseguradora, 
			porcentaje_desc,  publico, total,sub_total, itbms_total,
			descuento_total, efectivo,tarjeta_credito,tarjeta_clave,
			credito, saldo_pendiente, cheque, no_cheque, nombre_banco, 
		...
		public static function select2($id_articulo_$i) {
			$reg = conexion::sqlGet("select costo_unitario from medicamentos where codigo_interno = '".$id_articulo_$i."'");
			return $reg;
		}
		public static function insert2($g,$articulo_$i, $i,$identificacion,$id_articulo_$i,$cantidad_$i,$precio_unitario_$i, $itbms_unitario_$i, $descuento_unitario_$i, $precio_venta_$i,$costo_unitario ) {
			...
			precio_unitario, impuesto, descuento_unitario, precio_venta,
		...
		public static function select3($g) {
			$reg = conexion::sqlGet("SELECT a.factura, a.codigo_cliente, concat(b.nombre, ' ', b.apellido) as nombre,a.id_paciente as identificacion, a.fecha, a.ordenado_por, a.total, a.descuento_total, a.porcentaje_desc, a.efectivo, a.tarjeta_clave, a.tarjeta_credito, a.credito, a.cheque
		...
		public static function select5() {
			$reg = conexion::sqlGet("select nombre_carpeta, nombre_carpeta2 from impresoras_fiscales where tipo_impresion = 'PUB'");
			return $reg;
		}
