<?php
	...
	class enviar_furg{
		...
		public static function select1($factura) {
			$reg = conexion::sqlGet( "SELECT b.medicoFROM  factura b 
			WHERE b.factura = '".$factura."' and b.estado_factura = 'E'");
			return $reg;
		}
		public static function select2($medico) {
			$reg = conexion::sqlGet( "select nombre from medicos where codigo_medico =
			'".$medico."'");
			return $reg;
		}
		public static function select3($factura) {
			$reg = conexion::sqlGet("select usuario, sessionid from 
			temp_pend where factura= '".$factura."' " );
			return $reg;
		}
		...
		public static function select4($factura) {
			...
			a.nombre_paciente, a.no_cama, a.compania_de_seguro,
			d.edad, b.medico, a.contraindicaciones, a.tratamiento 
			as tratamiento, a.cargo as orden, b.despacho as despacho,
			b.cargo_impreso FROM registro a, factura b, tratamiento d
		...
		public static function select5($historia) {
			$reg = conexion::sqlGet("select historia, observacion, usuario, fecha_creacion from historias where historia =
			'".$historia."'");
			return $reg;
		}
		...
		public static function select7($factura) {
			...
			d.descripcion as desc_turno, a.lote, a.turno, 
			a.medicamento_id FROM factura_detalle a, 
			formas_farmaceuticas b, factura c, frecuencia_turno d
			where a.factura = '".$factura."' and estado_producto not in 
			('X', 'P') and a.forma_farma = b.codigo_forma and c.factura =
			a.factura and c.estado_factura in ('E') and a.turno =
			d.id_frecuencia_turno");
			return $reg;
		}
		public static function select8() {
			$reg = conexion::sqlGet("select lote_auto, ven_auto from
			lotes_parametros");
			return $reg;
		}
		public static function select9() {
			$reg = conexion::sqlGet("select codigo_razon, descripcion 
			from razon_observacion");
			return $reg;
		}
		public static function select10($medicamento_id) {
			$reg = conexion::sqlGet("select (cantidad_inicial - 
			cantidad_factura +
			cantidad_devolucion) as existencia from
			medicamentos_x_bodega where medicamento_id =
			'".$medicamento_id."'  and bodega = 1 ");
			return $reg;
		}
		public static function select11($medicamento_id) {
			$reg = conexion::sqlGet("select lote from 
			medicamentos_x_lote
			where medicamento_id = '".$medicamento_id."'
			and estado = 'A'  and cantidad > 0");
			return $reg;
		}
		public static function select12($medicamento_id) {
			$reg = conexion::sqlGet("select lote, fecha_vencimiento from
			medicamentos_x_lote where medicamento_id = 
			'".$medicamento_id."' and estado = 'A'  and cantidad >
			0 order by fecha_vencimiento asc limit 1");
			return $reg;
		}
