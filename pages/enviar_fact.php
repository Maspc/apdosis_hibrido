<?php 
	...
	require_once('../modulos/enviar_fact.php');
	...
	$fcies = enviar_fac::update1($_SESSION['MM_iduser'],$hora_actual,$factura); 
	
	
	$lres =enviar_fac::select1($factura);
	
	
	foreach($lres as $lrow)
	{
		$hist = $lrow->historia;
		$usuario_creacion = $lrow->usuario_creacion;
		$carg = $lrow->cargo;
	}
	...
	// Call mathod
	$result = $client->call('PacientesActivos_byHistoria', array('Historia' => $hist, 'UserId' => 'webapp', 'Session' => '2aa55c55'));
	// fault if any
	if ($client->fault) {
		...
		if ($err) {
			...
			//var_dump($result);
			if (isset($result['PacientesActivos_byHistoriaResult']['diffgram']['NewDataSet']['Table'])){
				$pacap1 = trim($result['PacientesActivos_byHistoriaResult']['diffgram']['NewDataSet']['Table']['pacap1']);
				$pachab = $result['PacientesActivos_byHistoriaResult']['diffgram']['NewDataSet']['Table']['pachab'];
				$pacnum = $result['PacientesActivos_byHistoriaResult']['diffgram']['NewDataSet']['Table']['pacnum'];
				
				} else if (isset($result['PacientesActivos_byHistoriaResult']['diffgram']['NewDataSet']['Table1'])) {
				header('location:error_salida.php?historia='.$hist.'&cargo='.$carg.'&usuario='.$usuario_creacion); 
				//echo "no encontre el historial y el estado de la sesion es: ".$estado_sesion;
			}   
			// echo '</pre>';
...
<html>
	<head>  
		...
		<?php
			...
			$result1 =enviar_fac::select2($factura); 
			...
			{ 
				...
				$feres =enviar_fac::select3($row1->medico);  
			...
			<div>
				...
				<font size="+1"> Orden de Entrega No. # <?php echo "$factura"; ?><br /> </font>
		...
		<font size="+1">
			<table border="0" bgcolor="#abd0fa" width="200" >
				<tr><td colspan="2" align="center">
					<?php
						...
						$rres =  enviar_fac::select4(); "select usuario, sessionid from temp_pend where factura= '$factura' ";
						...
						if ($rnum == 0 || $h == 1 || $usr == $_SESSION['MM_iduser']) {
							...
							$tres = enviar_fac::inser1($_SESSION['MM_iduser'], $factura, $id); 
							
							
							
							$result =enviar_fac::select4($factura); 
							...
							{ 
								...
								if(!is_null($pachab)){
									if ($row['no_cama'] != $pachab) {
										
										$rces =enviar_fac::update2($pachab,$row->cargo,$row->historia,$row->tratamiento);
										
										
										
										$fces = enviar_fac::update3($pachab,$factura);
								...
								$lres =enviar_fac::select5($row->historia.); 
								
								$lnum =count($lres);
								$oc = '0';
								
								//echo "Habitacion: ".$pachab;
								...
								$fres =enviar_fac::select6($row->medico); 
						...
						<tr bgcolor="#FFFFFF"><td> <b> Habitaci&oacute;n:</b></td><td> <?php echo $pachab; ?><br /></td></tr>
						<tr><td>  <b> Compa&ntilde;&iacute;a de Seguros:</b></td><td> <?php echo $row['compania_de_seguro']; ?></td></tr> </br>
				<tr bgcolor="#FFFFFF"><td>  <b> M&eacute;dico:</b> </td><td><?php echo $nom_med; ?> </td></tr> </p> 
			...
			<hr>
			<form action="imprimir_labels.php" method="post" target="_blank">
				<table> 
					<tr><td><strong>Medicamento</strong></td>
						...
						<td colspan="4"><strong>Fecha Venc. Prep.</strong></td>
						<td><strong>Contraindicaciones</strong></td>
						<td><strong>&iquest;Imprimir Labels? Todos? <input type='checkbox' name='checkall' onclick='checkedAll();'> </strong></td>
					...
					<?php 
						...
						$resulta =enviar_fac::select7($factura);
						...
						<tr class="<?php echo "$class"; ?>"><td><?php echo $rows->medicamento; ?></td>
							...
							<?php
								...
								$gres =  enviar_fac::select10($rows->medicamento_id);
								...
								if ($lote_auto == 'S') {
									...
									$gres =enviar_fac::select11($rows->medicamento_id); 
									...
									if ($gnum == 1) {	
										...
										$vres =enviar_fac::select12($rows->medicamento_id);
									...
									$gres = enviar_fac::select13($rows->medicamento_id);
						...
						$url = "finalizar.php?factura=".$factura;
						...
						if($num > 0) {
							...
							echo " <INPUT TYPE=\"button\" class='blue' value='Imprimir Cargo' id='imprimir' onClick=\" enable(); window.open('imprimir_factura.php?factura=".$factura."','mywindow','width=800,height=600');\" >
							<p>
							...
							<INPUT TYPE=\"submit\" id='imprimirlabel' name='label' value='Imprimir Label' onClick=\" enable2();\"  class = 'blue' disabled>
							<p>
							<INPUT TYPE=\"submit\" id='imprimirprep' name='prep' value='Imprimir Preparaci&oacute;n' onClick=\" enable2();\"  class = 'blue' disabled>
							<p>
