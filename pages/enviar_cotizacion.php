<?php 
	...
	require_once('../modulos/enviar_cotizacion.php');
	...
	*/
	/*
		...
		$b = "update clientes set saldo_actual = saldo_actual + '$credito' where id_cliente = '$codigo_cliente'";
	...
	$korow = ecotiza::select1($_SESSION['MM_iduser']);
	foreach($korow as $kw){
		$caja_id = $kw->caja_id;
		$nombre = $kw->nombre;
	}
	
	ecotiza::insert1($userid,$identificacion,$codigo_cliente,$jubilado,$codigo_aseguradora,$porcentaje_desc,$total,$sub_total,$itbms_total,$descuento_total,$efectivo,$tarjeta_credito,$clave,$credito,$saldo_pendiente,$cheque,$no_cheque,$nombres_banco,$ref_tdb,$ref_tdc,$vuelto,$caja_id,$nombre_cliente);
	...
	for ($i=1;$i<=$var_cont;$i++)
	{
		...
		if (isset($_POST["id_articulo_".$i])) {
			...
			*/
			...
			$vrow = ecotiza::select2($_POST["id_articulo_".$i]);
			foreach($vrow as $vw){
				$costo_unitario = $vw->costo_unitario;
			}
			
			ecotiza::insert2($g,$_POST["articulo_".$i],$i,$identificacion,$_POST["id_articulo_".$i],$_POST["cantidad_".$i],$_POST["precio_unitario_".$i],$_POST["itbms_unitario_".$i],$_POST["descuento_unitario_".$i],$_POST["precio_venta_".$i],$costo_unitario);
			/*
				$k = "update medicamentos_x_bodega set cantidad_factura = cantidad_factura + '".$_POST["cantidad_$i"]."' where bodega = '2' and medicamento_id = '".$_POST["id_articulo_$i"]."'";
				
			$kres = mysql_query($k, $conn) or die(mysql_error());*/
	    ...
	    $row = ecotiza::select3($z);
		foreach($row as $rw){
			$procesado_por = $rw->ordenado_por;
			$fecha_proceso = $rw->fecha;
		}
		
		$rowd = ecotiza::usuarios($procesado_por);
		foreach($rowd as $rod){
			$username = $rod->nombre;
		}
		
		if(empty($username)){
			$username = 'No Procesado por Apdosis';
		}
		...
		$pdf->Table("SELECT a.factura, a.codigo_cliente, a.nombre_cliente as Nombre,a.id_paciente as identificacion, a.fecha as Fecha, a.ordenado_por, a.total, a.descuento_total, a.porcentaje_desc, a.efectivo, a.tarjeta_clave, a.tarjeta_credito, a.credito, a.cheque
		from cotizacion a, clientes b where a.codigo_cliente = b.id_cliente and a.factura = '$g'", $prop3);
		$pdf->Ln(5);
		
		$pdf->Ln(5);
		//Second table: specify 3 columns
		$pdf->AddCol('medicamento',150,'Producto');
		$pdf->AddCol('cantidad',15,'Cantidad','C');
		$pdf->AddCol('precio_unitario',15,'Precio','C');
		$pdf->AddCol('impuesto',15,'Imp.','C');
		$pdf->AddCol('descuento_unitario',15,'Desc.','C');
		$pdf->AddCol('precio_venta',15,'Total','C');
		...
		$pdf->Table("SELECT a.medicamento, a.forma_farma, a.dosis_mostrar as dosis, a.horas, a.dias, a.precio_venta, a.linea, a.cantidad as cantidad, a.estado_producto, round(a.impuesto,2) as impuesto, round(a.descuento_unitario,2) as descuento_unitario, round(a.precio_unitario,2) as precio_unitario FROM cotizacion_detalle a where factura = '$z' ",$prop);
		
		
		$pdf->Ln(5);
		$pdf->Ln(5);
		$pdf->Write(5,'Atendido por: ');
		...
		$pdf->Ln(5);
		$pdf->Ln(5);
		$pdf->Write(5,'Hora de Proceso: '.$fecha_proceso);
		$pdf->Ln(5);
		...
		/*
			...
			</head> <body onunload=\"opener.location=('cotizacion.php')\">
			
			
			<center><h2>Previsualizaci&oacute;n de Factura</h2></center><p>";
			
			
			$m = "SELECT a.factura, a.codigo_cliente, concat(b.nombre, ' ', b.apellido) as nombre,a.id_paciente as identificacion, a.fecha, a.ordenado_por, a.total, a.descuento_total, a.porcentaje_desc, a.efectivo, a.tarjeta_clave, a.tarjeta_credito, a.credito, a.cheque
			from cotizacion a, clientes b where a.codigo_cliente = b.id_cliente and a.factura = '$g'";
			...
			$ko = "select caja_id from cajas_usuario where usuario = '".$_SESSION['MM_iduser']."'";
		...
		/*
			$pe = "select ruta_entrada, ruta_salida from cajas where caja_id = '$caja_id'";
			
			$respe = mysql_query($pe, $conn) or die(mysql_error());
			
			while ($rowspe = mysql_fetch_array($respe)){
		...
		*/
		...
		/*
			$n = "SELECT a.medicamento_id, a.medicamento, a.cantidad, a.precio_unitario,a.descuento_unitario, a.impuesto, a.precio_venta FROM factura_detalle a where factura = '$g'";
		...
		/*
			$c = "select (tipo_impuesto - 1) as tipo_impuesto from medicamentos where codigo_interno= '$id'";
